...
Assets::UniformBufferObject RayTracer::get_uniformBufferObjects(const VkExtent2D extent) const
{
...
void RayTracer::set_physical_device(
	...
	void* nextDeviceFeatures)
{
	...
	Application::set_physical_device(physicalDevice, requiredExtensions, deviceFeatures, &shaderClockFeatures);
}

void RayTracer::on_device_set()
{
	Application::on_device_set();

	LoadScene(userSettings_.SceneIndex);
	CreateAccelerationStructures();
}

void RayTracer::create_swapChain()
{
	Application::create_swapChain();

	userInterface_.reset(new UserInterface(get_commandPool(), get_swapChain(), get_depthBuffer(), userSettings_));
	resetAccumulation_ = true;

	CheckFramebufferSize();
}

void RayTracer::delete_swapChain()
{
...
void RayTracer::render(VkCommandBuffer commandBuffer, const uint32_t imageIndex)
{
	...
	// Render the UI
	Statistics stats = {};
	stats.FramebufferSize = get_window().get_framebuffer_size();
	...
	userInterface_->Render(commandBuffer, get_swapChainFrameBuffer(imageIndex), stats);
}

void RayTracer::on_key(int key, int scancode, int action, int mods)
{
	...
	{		
		switch (key)
		{
		case GLFW_KEY_ESCAPE: get_window().Close(); break;
		default: break;
		}
...
void RayTracer::on_cursorPosition(const double xpos, const double ypos)
{
	if (!HasSwapChain() ||
...
void RayTracer::on_mouseButton(const int button, const int action, const int mods)
{
	if (!HasSwapChain() || 
		userSettings_.Benchmark ||
		userInterface_->WantsToCaptureMouse())
...
void RayTracer::on_scroll(const double xoffset, const double yoffset)
{
	if (!HasSwapChain() ||
		userSettings_.Benchmark ||
		userInterface_->WantsToCaptureMouse())
...
void RayTracer::CheckAndUpdateBenchmarkState(double prevTime)
{
	...
	{
		const bool timeLimitReached = periodTotalFrames_ != 0 && get_window().get_time() - sceneInitialTime_ > userSettings_.BenchmarkMaxTime;
...
void RayTracer::CheckFramebufferSize() const
{
	...
	const auto& cfg = get_window().get_config();
	const auto fbSize = get_window().get_framebuffer_size();
