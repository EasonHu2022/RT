...
namespace Vulkan
{
	...
	void BufferUtil::CopyFromStagingBuffer(CommandPool& commandPool, Buffer& dstBuffer, const std::vector<T>& content)
	{
		...
		dstBuffer.copy(commandPool, *stagingBuffer, contentSize);
	...
	/*	const auto& debugUtils = device.DebugUtils();*/
		...
		memory.reset(new DeviceMemory(buffer->allocate_memory(allocateFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)));
